...
const CATEGORIES = [
...

  {
    ...
    accent: "#e6ecff",
    position: 4,
    description: "Third-party Roblox script executor listings and compatibility information.",
    children: ["Executors"]
  },

  {
    ...
    accent: "#b98cff",
    position: 5,
    description: "Third-party game cheat listings and compatibility information.",
    children: ["Cheats"]
  }
];
...
function homeBlocks() {
  return [
    ...
    { type: "categories", data: {
      ...
      text: "Explore our range of tools across different categories"
    } },

    { type: "featuredProducts", data: {
      title: "Featured Tools",
      text: "Handpicked tools you might need",
      limit: 6,
    ...
    { type: "productGrid", data: {
      ...
      text: "What the catalog is downloading this month",
      sort: "downloads",
      limit: 8
    } },
...
async function main() {
  ...
  const placeholders = await prisma.product.findMany({
    where: {
      ...
      downloadUrl: { startsWith: "https://example.com/" }
    },
    select: {
      id: true,
      name: true
  ...
  for (const item of attachLogos(CATALOG)) {
    ...
    const subcategoryId = item.sub
      ? categoryIds.get(`${item.category}-${item.sub.toLowerCase()}`)
      : null;

    if (!categoryId) {
      throw new Error(
        `Unknown category "${item.category}" for ${item.name}`
      );
    }

    if (item.sub && !subcategoryId) {
      throw new Error(
        `Unknown subcategory "${item.sub}" for ${item.name}`
      );
    }
  ...
  if (withDemoActivity && (await prisma.review.count()) === 0) {
    ...
    for (const [index, product] of products.entries()) {
      const [authorName, rating, title, body] =
        reviewSeeds[index % reviewSeeds.length];
  ...
  if (
    withDemoActivity &&
    (await prisma.downloadEvent.count()) === 0
  ) {
  ...
  const pages = [
    {
      slug: "home",
      title: "Home",
      seoTitle: "ToolHub — Ultimate Tools Collection",
      seoDescription:
        "A curated catalog of Windows, game, Roblox and crypto tools with verified details and official download links.",
      blocks: homeBlocks()
    },

    {
      slug: "windows",
      title: "Windows Tools",
      seoTitle: "Windows Tools — ToolHub",
      seoDescription:
        "System utilities and productivity tools for Windows.",
      blocks: [
        {
          type: "hero",
          data: {
            ...
            titleAccent: "Tools",
            subtitle:
              "Essential utilities and productivity tools for Windows.",
            variant: "compact"
          }
        },
        {
...

    {
      slug: "game",
      title: "Game Tools",
      seoTitle: "Game Tools — ToolHub",
      seoDescription:
        "Utilities and tools for your favourite games.",
      blocks: [
        {
          type: "hero",
          data: {
            ...
            titleAccent: "Tools",
            subtitle:
              "Discover utilities and tools for your favourite games.",
            variant: "compact"
          }
        },
        {
...

    {
      slug: "roblox",
      title: "Roblox Tools",
      seoTitle: "Roblox Tools — ToolHub",
      seoDescription:
        "Studio helpers and utilities for Roblox creators.",
      blocks: [
        {
          type: "hero",
          data: {
            ...
            titleAccent: "Tools",
            subtitle:
              "Studio helpers and quality-of-life tools for Roblox creators.",
            variant: "compact"
          }
        },
        {
...

    {
      slug: "crypto",
      title: "Crypto Tools",
      seoTitle: "Crypto Tools — ToolHub",
      seoDescription:
        "Wallets, analytics and trading utilities.",
      blocks: [
        {
          type: "hero",
          data: {
            ...
            titleAccent: "Tools",
            subtitle:
              "Wallets, portfolio trackers and on-chain analytics.",
            variant: "compact"
          }
        },
        {
...

    {
      ...
      seoTitle: "FAQ — ToolHub",
      seoDescription:
        "Answers to common questions about the ToolHub catalog.",
      blocks: [
        {
          type: "hero",
          data: {
...

    {
      ...
      seoDescription:
        "Why ToolHub exists and how the catalog is maintained.",
      blocks: [
        {
          type: "hero",
          data: {
            eyebrow: "ABOUT TOOLHUB",
            title: "A better way to discover",
            titleAccent: "useful tools.",
            subtitle:
              "ToolHub is a directory built around clear product information, fast discovery and a calm dark interface.",
            variant: "compact"
          }
        },

        {
          ...
          data: {
            title: "Our mission",
            body:
              "Make software discovery feel organized instead of overwhelming. Every product gets the same consistent card, a detailed page and metadata worth reading: real version numbers, file sizes, requirements and a changelog."
          }
        },

        {
          type: "text",
          data: {
            title: "How the catalog is maintained",
            body:
              "Entries are reviewed before publication and refreshed whenever a vendor ships a release. Downloads always redirect to the official source, so you are never installing something re-hosted here.",
            columns: 2,
            bullets: [
              "Curated, reviewed entries",
              "Version and changelog tracking",
              "Official download links only",
              "Moderated visitor reviews",
              "No visitor accounts, no tracking cookies"
            ]
          }
        },
...

        {
          ...
          data: {
            ...
            text:
              "Tell us which tool deserves a place in the catalog.",
...
async function seedDemoEvents(products) {
  ...
  const pool = weights.flatMap(([code, weight]) =>
  ...
  for (let dayOffset = 89; dayOffset >= 0; dayOffset -= 1) {
    ...
    const base =
      18 + Math.round((90 - dayOffset) / 4);

    const weekend =
      [0, 6].includes(day.getDay()) ? 0.7 : 1;
  ...
  console.log(
    `  demo activity: ${downloads.length} downloads, ${views.length} views`
  );
